...
export default function RolePicker({ roles, onChange }) {
  ...
  const rolesFor = (domainName) => domains.find((d) => d.name === domainName)?.roles.map((r) => r.name) || [];

  // Keep a value visible in its select even if the taxonomy no longer lists it
  // (e.g. editing a person whose domain/role was renamed or removed).
  ...
  const updateRole = (index, field, value) => {
    ...
    if (field === 'domain') {
      updated[index].role = rolesFor(value)[0] || '';
    }
    onChange(updated);
  };
  ...
  const addRole = () => {
    ...
    onChange([...roles, { domain, role: rolesFor(domain)[0] || '', seniority: seniorityNames[0] || '' }]);
  };
      ...
      {roles.map((r, i) => (
        <div key={i} className="flex gap-2 mb-2 items-center">
          <select value={r.domain} onChange={(e) => updateRole(i, 'domain', e.target.value)}
            ...
            {withCurrent(domainNames, r.domain).map((d) => <option key={d} value={d}>{d}</option>)}
          </select>
          <select value={r.role} onChange={(e) => updateRole(i, 'role', e.target.value)}
            className="px-2 py-1.5 border border-border rounded-lg text-xs text-text bg-white">
            {withCurrent(rolesFor(r.domain), r.role).map((rl) => <option key={rl} value={rl}>{rl}</option>)}
          </select>
          <select value={r.seniority} onChange={(e) => updateRole(i, 'seniority', e.target.value)}
            className="px-2 py-1.5 border border-border rounded-lg text-xs text-text bg-white">
            {withCurrent(seniorityNames, r.seniority).map((s) => <option key={s} value={s}>{s}</option>)}
          </select>
          {roles.length > 1 && (
