  ...
  return (
    <div className="bg-white rounded-2xl border border-border-light shadow-card overflow-auto">
      <h3 className="text-[13px] font-bold text-text px-5 pt-4 pb-2">
        Capacity Heatmap{' '}
        <InfoDot text="Each month is a small bullet: the soft track is the person’s realised allocation as % of capacity with a tick at its target (red when over 100%), the solid bar is the utilization they actually logged in Tempo — stacked by work type (green = client, violet = internal) — and the label reads actual/planned. The Hours filter switches the actual layer between all work, one work type, or absences (absences never count as worked time elsewhere). Click a row to break the person’s hours down per client, internal work and absences; click their name to open the profile. Rows share a scale anchored at 100% capacity; people without synced hours get no actual layer. A grey bar is the current month, still being logged." />
