...
export const projectRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.post('/projects', { preHandler: requirePermission('customers', 'create') }, async (req) => {
    const data = createProjectSchema.parse(req.body);
    await assertResponsibleUserAllowed(req.orgId, data.responsibleUserId);
    return projectService.create(req.orgId, data);
  });

  app.patch('/projects/:id', { preHandler: requirePermission('customers', 'edit') }, async (req) => {
  ...
  app.delete('/projects/:id', { preHandler: requirePermission('customers', 'delete') }, async (req, reply) => {
    const { id } = req.params as { id: string };
