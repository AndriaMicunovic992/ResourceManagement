...
export const logRoutes: FastifyPluginAsync = async (app) => {
  app.get('/people/:personId/logs', { preHandler: requireRole('admin') }, async (req) => {
  ...
  app.post(
    ...
    async (req, reply) => {
      ...
      const created = await createLog(req.orgId, personId, req.userId, body);
  ...
  app.patch(
    ...
    async (req) => {
      ...
      return updateLog(req.orgId, id, req.userId, body);
    }
  );
