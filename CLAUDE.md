...
- **Substitution** (long leaves): the bar's FTE popover has **Substitute…** → `SubstitutePopover.jsx` picks a from-month (defaults to the clicked one) and hands **all remaining months** to a role-matched candidate (ranked by free *effective* capacity; takes over at the original person's FTE, merged on top of any months they already hold on that need) or to **nobody** ("no substitute" just zeroes them, reopening the need). Substitute-first write order, one undo entry for the whole handover.
