...
- **Where actuals show:** cockpit charts, the home dashboard's utilization line/KPI, and the **Insights** heatmaps. The Client-Staffing and People-Capacity heatmaps render a small **"act"** line under the plan for elapsed months (`features/dashboard/useWindowActuals.js` fetches per-customer/project/person Tempo hours for the window; colored by `actualVsPlanColor` — green ≈ on plan, amber under, red over). Rows without any synced hours get no act line (instead of misleading zeros), and the client heatmap hides the act layer while a team filter is on (logged hours aren't team-attributable). Future months are plan-only by nature.
