...
- **Where actuals show:** cockpit charts, the home dashboard's utilization line/KPI, and the **Insights** heatmaps. The Client-Staffing and People-Capacity heatmaps render each month as a **bullet cell** (`features/dashboard/BulletCell.jsx`): soft accent track = plan with a tick at its target, solid green fill = logged Tempo hours, small mono `act/plan` label underneath, portal tooltip (`Tip.jsx`) with needed/planned/actual/Δ. Bar lengths are normalized per row (client rows floor the scale at 1 FTE; people rows anchor at 100% capacity). The tick turns **red** when the plan itself needs attention (client: filled < needed; person: > 100% capacity); an amber fill over a dashed baseline is **unplanned work**; the current month renders grey ("still logging"). `useWindowActuals.js` fetches per-customer/project/person hours for the window; rows without synced hours get no actual layer (never zeros), and the client heatmap hides the actual layer while a team filter is on (hours aren't team-attributable).
