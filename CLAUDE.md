...
- **Absence-adjusted expectations** (`client/src/lib/availability.js`): planning someone 1.0 means "all their working time", so all Insights **Δs/verdicts and the Actual-vs-plan KPI** judge actuals against **expected** = plan × availability, where availability = (capacity hours − synced absence hours) ÷ capacity hours per person-month, pro-rated across that person's planned clients. Displayed %s and bar geometry stay capacity-based (so they match the visuals); tooltips show the Expected value whenever absences reduce it. Future months always ride at full availability (no absence actuals by design); missing absence data degrades to expected = plan.
