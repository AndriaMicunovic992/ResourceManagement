...
- **Integrations (Jira/Tempo) → actual hours.** `JiraConnection` holds per-org Jira/Tempo config with **encrypted** tokens (AES-256-GCM, `utils/crypto.ts` — never return token plaintext). `JiraAccount` caches Jira users; a person is **"matched"** when `Resource.externalWorkId` holds their Jira `accountId`. Synced Tempo worklogs land in `Worklog` (one row per `tempoWorklogId`, **idempotent and batched** — rows are resolved first, then written via chunked `createMany`/`updateMany` so a large first pull doesn't run past proxy timeouts), each resolved to `{ resourceId?, customerId?, projectId? }` + the Jira issue/epic; `month` is a `"YYYY-MM"` string. `services/integration.service.ts` owns the sync (`syncHours`) and the read aggregations; `JiraWorkItem` maps Jira epics/projects → customers/projects **or classifies them via `workType`** (`client | internal | absence` — non-client items are org-level buckets and never carry a customer/project; the service enforces the exclusivity and the sync stamps the resolved `workType` onto each `Worklog`). **Absence hours are excluded from the per-person actuals** (`actualsByResource`) so time off never counts as logged work; internal hours count as worked time but roll into no customer. **Mapping edits re-attribute history**: every work-item create/update/delete (and the end of every sync) runs `restampWorklogs`, which re-resolves all stored worklogs through the current mappings — a delta sync never re-pulls old rows, so without this a reclassified Jira project would keep its old attribution forever. Rows with no Jira keys (hand-attributed) are left alone. The **actuals read endpoints** (visibility-scoped, not admin-only) are `GET /integration/tempo/actuals` (one customer+month → per person), `/actuals/monthly` (per person/month, absences excluded), `/actuals/monthly-by-type` (per person/month/workType, unfiltered — feeds the people heatmap's stacked bar, its Hours filter, and the drill-down buckets), `/actuals/monthly-by-customer`, `/actuals/monthly-by-project`, `/actuals/resource-by-customer` (one person → by customer; powers the heatmap drill-down), `/actuals/customer-by-resource` (one customer → by person). **FTE↔hours** uses `MONTHLY_HOURS_PER_FTE` (≈173.33, `client/src/lib/constants.js`).
...
- **Planned absences (days off) live on the planner.** `Resource.plannedAbsences` is a `{ "YYYY-MM": days }` JSON (workdays, halves allowed; 1 d = 8 h; full month ≈ 21.7 d), edited via the calendar button on a pool card (`pool/AbsencePopover.jsx`) → `PATCH /resources/:id/absences` (planner-edit gated; per-month **merge**, 0 clears the key — mirror of assignment month semantics). **Effective capacity** = capacity − planned-absence FTE (`client/src/lib/availability.js effectiveCapacity`) is what the planner books against: `buildAutoFill`, the held-resource footer, pool free numbers ("off" when fully away), suggestion ranking, the Insights Free-capacity tab, and the **red overload tick** (allocation > effective capacity — a fully-off month flags any allocation, which is the "needs a substitute" cue). Bar months with days off get a **slate hatch strip**; the pool card shows an "N d off" chip.
- **Substitution**: the bar's FTE popover has **Substitute…** → `SubstitutePopover.jsx` picks a month (defaults to the clicked one), a **scope** — **only that month** (default; short leaves/rebalances) or **month → end** (long leaves) — and an **amount** in FTE/hours (defaults to the person's full allocation; capped per month at what they hold). A partial amount **splits the month**: the original keeps the rest, the substitute takes the amount on top of anything they already hold on that need. Candidates are role-matched, ranked by free *effective* capacity over the handed months; **nobody** ("no substitute") removes the amount and reopens the need. Substitute-first write order, one undo entry for the whole handover.
...
- **Absence-adjusted expectations** (`client/src/lib/availability.js`): absences subtract from **max capacity and eat the person's slack first** — while the hours left still cover their plan, expected = plan. Only once available hours (capacity − absences) fall below the person's TOTAL plan does the expectation shrink, to the available hours, spread **pro-rata across their engagements** (`deliverableRatio(planTotalHours, availH)`). All Insights **Δs/verdicts and the Actual-vs-plan KPI** (shown as a **±deviation**, e.g. +43%) judge actuals against that expected. Displayed %s and bar geometry stay capacity-based (so they match the visuals); tooltips show the Expected value only when the absence actually bites into the plan. **Elapsed months use synced Tempo absences; current/future months use the planned days off entered in the planner** (`plannedAvailabilityRatio`) — that's where the client/project heatmap's forward Expected, the people heatmap's forward red tick ("over available capacity — N d off") and its Days-off tooltip row come from. On the people heatmap the forward **plan line clips to the workable part** (capacity − days off; the % label follows): the clipped remainder renders as a **slate hatch** up to the tick, which stays at the full booking — so an overextended plan is visible without pretending the person is deliverably booked. Missing absence data degrades to expected = plan.
- **Where actuals show:** cockpit charts, the home dashboard's utilization chart (a teal **planned line** for matched people vs **stacked monthly bars** of logged hours — client / unmapped / internal / absences via `/actuals/monthly-work-buckets`; falls back to the plan-lines view when nothing is synced; tooltip values are **hours with the % alongside**). The home dashboard has a **team selector** scoping the capacity-side KPIs, donut and chart to one team — team bars are summed from per-person typed hours (`/actuals/monthly-by-type`), so the unmapped split folds into Client; demand-side KPIs (Unfilled, Active projects) stay org-wide. Team membership is editable in Settings → Teams (member chips + add-select; writes `updateResource(teamIds)`). And the **Insights** tables (titled **Client Staffing** / **People Capacity** — the component files still carry their old `*Heatmap*` names). Both render each month as a **bullet cell** (`features/dashboard/BulletCell.jsx`): soft accent track = plan with a tick at its target, solid fill = logged Tempo hours, small mono `act/plan` label underneath, portal tooltip (`Tip.jsx`) with needed/planned/actual/Δ. On the people heatmap the fill is **stacked by work type** (green = client, violet = internal, slate = absences — absences render in the stack so time off explains plan shortfalls, but never count toward the act number; `WORK_TYPE_COLORS`) with an **Hours filter** (all work / client / internal / absences, `workTypeLens.js`), and **rows expand to a per-client drill-down** (`ResourceBreakdownRows.jsx`: plan-vs-logged hours per customer + Internal/Absences bucket rows + one **"not mapped"-flagged row per unmapped Jira project** via `/actuals/resource-unmapped`; per-customer hours lazy-load via `/actuals/resource-by-customer`). Every drill row with logged hours goes **two levels deeper — Jira epics, then issues** (`/actuals/resource-epics`, scope = customer | internal/absence bucket | unmapped project): actual hours only (no plan exists at that depth), epic names from `JiraWorkItem`, issues keyed with the most recent worklog note as a description hint. Bar lengths are normalized per row (client rows floor the scale at 1 FTE; people rows anchor at 100% capacity; drill-down uses a shared hour scale). The tick turns **red** when the plan itself needs attention (client: filled < needed; person: > 100% capacity); an amber fill over a dashed baseline is **unplanned work**; the current month renders grey ("still logging"). `useWindowActuals.js` fetches per-customer/project/person(/type) hours for the window; rows without synced hours get no actual layer (never zeros). The Insights **team filter scopes the client table's actuals too**: `/actuals/monthly-by-customer|by-project` take an optional `teamId` (resolved server-side to the team's people; unmatched people's hours drop out). Expanding a customer drills **into the logged hours, not the plan**: its Jira **epics → tasks → the people who logged them** (`/actuals/customer-epics`, team-aware, per-issue people filtered to the requester's visible people; same `EpicDrillRows` component as the people drill). Project/need rows are gone from that drill — Jira work rarely maps 1:1 onto planned needs — the plan is reached via the **plan →** link under each customer name (planner deep-link `?customerId=`), profile via **profile →** or the name.
